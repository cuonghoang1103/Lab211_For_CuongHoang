...
>
> Theo **tờ checklist giấy** (21/09/2026), cùng khuôn P0054: **một** `WorkerRequestDTO`, **một**
> `WorkerResponseDTO` (`message` + `rowList`), view nhận qua **thuộc tính** rồi `display()` **1 lần/luồng**,
> interface tên `ISalaryStrategy` (1.3), `Main` là `final` + constructor `private` (3.4).
...
| `changeSalary(SalaryStatus status, String code, double amount)` | 3 tham số | `WorkerService.changeSalary(WorkerRequestDTO)` — **giữ tên, gói 3 giá trị vào DTO** |
| `List<SalaryHistory> getInfomationSalary()` | *"sort by id"* | `WorkerService.getInfomationSalary()` — giữ **đúng chính tả của đề** (`Infomation`), trả `ArrayList<String>` (các dòng bảng đã sắp) |
...
`historyList.sort(Comparator.comparing(history -> history.getWorker().getCode()))` — sắp theo code.
...
| `enum SalaryStatus { UP, DOWN }` | `%s` của enum in đúng tên hằng `UP`/`DOWN` ở cột Status |
...
├── dto/         WorkerRequestDTO        5 ô của Add + status, amount của Up/Down   (main ──► controller)
│                WorkerResponseDTO       message + rowList                           (controller ──► view)
...
│                ISalaryStrategy         «interface» calculateSalary(salary, amount)
│                IncreaseSalaryStrategy  salary + amount
│                DecreaseSalaryStrategy  salary − amount
├── controller/  WorkerController        Facade: addWorker · changeSalary · displaySalaryHistory
├── view/        WorkerView              responseDTO · setResponseDTO · display()
...
|---|---|---|
| `WorkerRepository` | lưu + tìm (`isExistWorker`, `addWorker(requestDTO)` dựng `Worker`, `findWorker`, `addHistory`, `getHistoryList`) | Guide: repository = **giữ dữ liệu + CRUD** |
...
```
Main: inputSalary(sc, UP) ──► WorkerRequestDTO ──► controller.changeSalary(requestDTO)   ← 1 lần/case
   controller ──► service.changeSalary(requestDTO)
                     ├─ repository.findWorker(code)                → null? throw "Code [..] does not exist."
                     ├─ amount <= 0?                                → throw "Amount of money must be > 0"
                     ├─ strategyMap.get(UP).calculateSalary(...)    → IncreaseSalaryStrategy
                     ├─ newSalary <= 0?                             → throw "Salary must be greater than 0"
                     └─ worker.setSalary + repository.addHistory(new SalaryHistory(..., LocalDate.now()))
   controller ──► responseDTO.setMessage("Salary has been adjusted.") ──► view.setResponseDTO ──► view.display()
...
| **Solution** | `ISalaryStrategy` = **Strategy** (`double calculateSalary(double salary, double amount)`). `IncreaseSalaryStrategy`, `DecreaseSalaryStrategy` = **ConcreteStrategy**. `WorkerService` = **Context**: giữ `HashMap<SalaryStatus, ISalaryStrategy> strategyMap` (tạo + điền trong constructor) và gọi `strategy.calculateSalary(...)` mà không biết là cộng hay trừ. |
...
**Thầy bảo "thêm tăng lương theo %":** thêm `BONUS` vào `SalaryStatus`, tạo `PercentSalaryStrategy implements ISalaryStrategy`
...
| Nguyên lý | Ở đâu |
|---|---|
| **S** | `Worker` mô tả · `WorkerRepository` lưu · `WorkerService` luật · `WorkerView` in · `Validation` kiểm dạng · `Main` nhập |
| **O** | kiểu điều chỉnh mới không sửa `changeSalary` (3.1) |
| **L** | `IncreaseSalaryStrategy`/`DecreaseSalaryStrategy` thay nhau được chỗ `ISalaryStrategy` |
| **I** | `ISalaryStrategy` chỉ 1 hàm `calculateSalary` |
| **D** | `changeSalary` phụ thuộc interface `ISalaryStrategy`, không phụ thuộc lớp cụ thể |
...
| 2 | `model/Worker.java` | 5 field `private` + ctor rỗng + ctor đủ + get/set + `toString` (`String.format(Constants.WORKER_FORMAT, …)`) |
| 3 | `model/SalaryHistory.java` | `Worker worker`, `double salary`, `SalaryStatus status`, `LocalDate date` + `toString()` = **1 dòng bảng** (`Constants.ROW_FORMAT`) |
| 4 | `dto/` 2 lớp | `WorkerRequestDTO` (7 field: 5 ô Add + `status`, `amount`), `WorkerResponseDTO` (`message` + `ArrayList<String> rowList`) |
| 5 | `repository/WorkerRepository.java` | `LinkedHashMap` + `ArrayList` (tạo trong constructor) + 5 hàm CRUD |
| 6 | `service/ISalaryStrategy` + 2 lớp con | mỗi lớp 1 dòng `return` |
| 7 | `service/WorkerService.java` | constructor tạo `strategyMap` + `put` 2 strategy; `addWorker`, `changeSalary`, `getInfomationSalary` |
| 8 | `constants/Message.java`, `Constants.java` | prompt/lỗi **chép đúng**; `MIN_AGE=18`, `MAX_AGE=50`, định dạng |
| 9 | `view/WorkerView.java`, `controller/WorkerController.java` | `responseDTO` + `setResponseDTO` + `display()`; 3 hàm điều hướng, mỗi hàm render **1 lần** |
...
| Việc | Cách làm |
|---|---|
| Breakpoint | dòng `newSalary = strategy.calculateSalary(...)` trong `WorkerService.changeSalary` |
...
| Bước | **F7** vào `calculateSalary` → nhảy vào `IncreaseSalaryStrategy` (đa hình qua interface); làm lại với option 3 → nhảy vào `DecreaseSalaryStrategy` |
...
| Câu hỏi | Trả lời mẫu |
|---|---|
| 4 tính chất OOP ở đâu? | **Đóng gói**: field `private` trong `Worker`, `SalaryHistory`, DTO + get/set. **Kế thừa**: `IncreaseSalaryStrategy implements ISalaryStrategy`; mọi lớp `extends Object`, em ghi đè `toString()`. **Đa hình**: `strategy.calculateSalary(...)` — biến kiểu interface, chạy bản của lớp thật; `history.toString()` chạy bản của `SalaryHistory`. **Trừu tượng**: interface `ISalaryStrategy` chỉ nói "tính được lương mới"; `Main` gọi `controller.changeSalary(requestDTO)` không biết dữ liệu nằm đâu. |
...
| `WorkerService` 3 hàm đề bắt | `public` — `WorkerController` gọi |
| `calculateSalary` trong strategy | `public` — hàm của interface luôn public |
| `WorkerController` 3 hàm, `WorkerView.setResponseDTO/display` | `public` — `Main` / controller gọi |
...
| `getInfomationSalary` trả `ArrayList<String>`, không `List<SalaryHistory>`? | Controller **không được làm với model** (tờ giấy 1.1) → service trả các **dòng chữ** (mỗi dòng = `SalaryHistory.toString()`, Guide: *"Cần output gì thì thêm hàm toString()"*); controller bỏ vào `WorkerResponseDTO.rowList`. |
...
| Sao `changeSalary` nhận DTO? | Thầy: *"không được truyền 3 tham số 1 hàm"* → gói `status, code, amount` vào `WorkerRequestDTO` (một DTO cho cả 3 luồng nhập, theo khuôn P0054). |
...
| **View nhận dữ liệu thế nào?** | Qua **thuộc tính**: `WorkerView` có `private WorkerResponseDTO responseDTO` + `setResponseDTO(...)`; `display()` **không tham số**, in `message` nếu có, in bảng nếu có `rowList`. Mỗi hàm của controller gọi `setResponseDTO` rồi `display()` **đúng 1 lần**. Bản trước có `showMessage(String)` (nhận qua tham số) — đã bỏ. |
| **Validate ở đâu?** | **Dạng** dữ liệu (có phải số không, menu 1–5) ở `Main` qua `utils/Validation` — sai thì hỏi lại ngay ô đó. **Luật của đề** (code trống/trùng/không có, tuổi 18–50, lương/số tiền > 0) ở `WorkerService`: đề đặt chúng trong `addWorker … throws Exception`, nên lỗi hiện **sau khi nhập đủ** — đúng màn hình tham chiếu. |
| Main gọi controller mấy lần mỗi lựa chọn? | **1 lần** mỗi `case` (Guide: *"Mỗi workflow chính chỉ gọi vào controller 1 lần duy nhất"*). |
| Sao hàm controller của option 4 tên `displaySalaryHistory`, không `getInfomationSalary`? | Hàm đó **không trả gì** (`void`) — nó cho view hiện bảng. Tên bắt đầu bằng `get` mà không trả gì là sai nghĩa (1.4). Tên của đề `getInfomationSalary` vẫn giữ ở **service** — nơi thật sự trả danh sách. |

### Tờ checklist 25 mục — bài này đạt thế nào

| Mục | Chỉ vào đâu |
|---|---|
| **1.1** MVC + repository | `repository/WorkerRepository`; `WorkerController` không import `model`; `WorkerView` nhận `responseDTO` qua setter, `display()` không tham số, gọi **1 lần/luồng**; mỗi `case` trong `Main.main` gọi controller **1 lần** |
| **1.3** interface bắt đầu bằng `I` | `service/ISalaryStrategy` (bản trước `SalaryStrategy`) |
| **1.4** method mở đầu bằng động từ | `calculateSalary` (bản trước `adjust`), `displaySalaryHistory`, `isExistWorker`, `findWorker` |
| **1.5** tên collection / map | `workerMap`, `historyList`, `strategyMap`, `rowList` (bản trước `histories`, `result`) |
| **2.6 / 3.7** khai báo đầu block + khởi tạo | `Main.main`: `requestDTO = null`, `running = true`, `choice = 0`; `Main.inputX`: `String line = ""`, trong vòng lặp chỉ gán; `WorkerService.changeSalary`: `worker`, `strategy`, `history = null`, `newSalary = 0` ở đầu hàm; `Validation.getDouble`: `double value = 0` |
| **2.8** dòng trống | giữa các field/hằng enum (mọi lớp), sau vùng khai báo biến, trước mọi comment đứng sau dòng code, giữa các `case`, sau `}` trước câu lệnh tiếp |
| **3.3** ngoặc | `WorkerService.addWorker`: `if ((code == null) \|\| code.isEmpty())`, `if ((age < MIN_AGE) \|\|` xuống dòng **sau** `\|\|`; `Validation.getChoice`: `if ((choice < min) \|\| (choice > max))` |
| **3.4** lớp chỉ có static | `Main` (`final` + `private Main()`), `Validation`, `FormatUtils`, `Constants`, `Message` |
| **3.8** cộng chuỗi | `Worker.toString()`, `SalaryHistory.toString()` dùng `String.format(Constants.…_FORMAT, …)` (bản trước nối `+`) |
...
|---|---|---|
| Thêm kiểu điều chỉnh (thưởng %) | `SalaryStatus`, 1 class `implements ISalaryStrategy` mới, 1 dòng `put` trong constructor `WorkerService`, `Message.MENU`, `Constants`, 1 `case` ở `Main` | `changeSalary`, repository, view |
| Sắp bảng theo tên | 1 dòng `Comparator` trong `getInfomationSalary` | mọi file khác |
| Tuổi 18–60 | `Constants.MAX_AGE` | mọi file khác |
| Hiện cả work location trong bảng | `SalaryHistory.toString()` (thêm `worker.getWorkLocation()`), `Constants.ROW/HEADER_FORMAT`, `Message` (nhãn), `WorkerView` (header) | `Main`, controller, service, repository, DTO |
...
|---|---|---|---|
| `changeSalary(status, code, amount)` | 3 tham số | `changeSalary(WorkerRequestDTO)` | luật V4 — giữ tên hàm |
| `addWorker(Worker worker)` | nhận model | `addWorker(WorkerRequestDTO)` | Guide: dữ liệu vào controller qua DTO |
| `List<SalaryHistory> getInfomationSalary()` | trả model | `ArrayList<String>` (mỗi phần tử = `SalaryHistory.toString()`, đã sắp theo code) | controller không được làm với model; V5 kiểu cụ thể |
...
| Tên interface | bản 14/09: `SalaryStrategy`, hàm `adjust` | `ISalaryStrategy`, hàm `calculateSalary` | tờ giấy 1.3 (interface bắt đầu bằng `I`), 1.4 (method bắt đầu bằng động từ) |
| View | bản 14/09: `setHistoryList(ArrayList<…>)` + `display()` + `showMessage(String)` | `setResponseDTO(WorkerResponseDTO)` + `display()` duy nhất | tờ giấy 1.1: view nhận qua **thuộc tính (ResponseDTO)**, render 1 lần/luồng |
| DTO | bản 14/09: `WorkerRequestDTO`, `SalaryRequestDTO`, `SalaryHistoryResponseDTO` (1 dòng, có `toString`) | **một** `WorkerRequestDTO` + **một** `WorkerResponseDTO` (`message` + `rowList`); chữ của dòng bảng nằm ở `SalaryHistory.toString()` | khuôn chung của kho (P0054) |
| Tiêu đề bảng `----Display Information Salary----` | bản 14/09: view in | `Main` in trước khi gọi controller (như tiêu đề Add, Up/Down) | cùng khuôn P0054; màn hình **không đổi** |
| Hàm controller option 4 | bản 14/09: `getInfomationSalary()` trả `void` | `displaySalaryHistory()` | tờ giấy 1.4: tên phải đúng việc (không trả gì thì không bắt đầu bằng `get`) |
| Tạo `Worker` | bản 14/09: service `new Worker(...)` | `WorkerRepository.addWorker(requestDTO)` | repository = Create (khuôn P0054/P0070) |
| Nối chuỗi trong model | bản 14/09: `code + " - " + name`, `… + " " + …` | `String.format` | tờ giấy 3.8 |
